...
const categories = ['All', 'Home Health', 'Hospice', 'Medical Billing', 'Coding', 'RCM', 'Credentialing', 'Healthcare Operations', 'Technology & AI']
...
const posts = [
  ...
  { category: 'Technology & AI', title: 'Technology-Enabled RCM: A Modern Approach', date: 'Jul 28, 2026', readTime: '9 min read', excerpt: 'How healthcare organizations are leveraging technology to improve revenue cycle visibility and efficiency.' },
  ...
  { category: 'Healthcare Operations', title: 'Denial Management Best Practices', date: 'Jul 5, 2026', readTime: '6 min read', excerpt: 'Proactive strategies for identifying, preventing, and appealing claim denials.' },
]
    ...
    <>
      ...
      <section className="relative min-h-[60vh] flex items-center bg-mbx-navy overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div className="absolute top-1/2 -right-32 h-[400px] w-[400px] rounded-full bg-mbx-teal/5 blur-[120px]" />
        </div>
      ...
      {/* Filters */}
      <section className="sticky top-[72px] z-30 bg-mbx-white/95 backdrop-blur-md border-b border-mbx-border py-4">
        <div className="container mx-auto">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-xs font-bold transition-all ${
                    activeCategory === cat
                      ? 'bg-mbx-navy text-mbx-white shadow-md'
            ...
            <div className="relative">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-mbx-text-muted" />
              <input
                type="text"
                placeholder="Search articles..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full rounded-lg border border-mbx-border bg-mbx-surface pl-9 pr-4 py-2.5 text-sm outline-none focus:border-mbx-teal focus:ring-1 focus:ring-mbx-teal/20 transition-all font-medium sm:w-64"
              />
            </div>
          </div>
        </div>
            ...
            <StaggerChildren className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3" stagger={0.06}>
              {filtered.map((post, index) => (
                <StaggerItem key={index}>
                  <article className="group flex h-full flex-col rounded-2xl border border-mbx-border bg-mbx-white p-7 transition-all duration-500 hover:shadow-xl hover:border-mbx-teal/30 hover:-translate-y-1">
                    <div className="mb-4 flex items-center gap-3">
                      <span className="rounded-full bg-mbx-teal/10 px-3 py-1 text-xs font-bold text-mbx-teal">
                        {post.category}
                      </span>
                    ...
                    <h3 className="mb-3 text-lg font-extrabold text-mbx-navy group-hover:text-mbx-teal transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-mbx-text-muted font-medium">{post.readTime}</span>
                      <span className="inline-flex items-center gap-1 text-sm font-bold text-mbx-teal">
